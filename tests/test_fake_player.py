...
class TestFakePlayerByDefault:
    ...
    def test_fake_starts_stopped(self, widget: PlaybackWidget):
        assert widget.is_playing() is False
        assert widget.is_player_active() is False
        assert widget.player.paused is False
        assert widget.player.curr_pos == 0.0
        assert widget.player.duration > 0

    def test_fake_tracks_play_pause_stop(self, widget: PlaybackWidget):
        ...
        assert widget.is_playing() is False
        assert widget.is_player_active() is True
        assert widget.player.paused is True
    ...
    def test_fake_seek_moves_position(self, widget: PlaybackWidget):
        widget.player.curr_pos = 1.0
        widget.seek_relative(2)
        assert widget.player.curr_pos == 3.0

    def test_fake_records_calls_like_a_mock(self, widget: PlaybackWidget):
        widget._current_path = "test.mp3"
        widget.play()
        widget.player.play.assert_called_once()
