...
2. `just release` then opens the pull request with your `gh` login, so CI runs on it. From the Actions tab, open it from the link in the run summary instead (or add a `RELEASE_PR_TOKEN` repository secret holding a personal access token and the workflow opens it). Review the changelog and merge like any other PR.
